...
public class Tree<T extends Comparable<T>> {
    ...
    private Tree right;
    private Tree parent;
    private ArrayList<T> listForPrint = new ArrayList<>();
    public T val() {
        return val;
    }
    public Tree left() {
    ...
    public ArrayList<T> getListForPrint()
    {
        return listForPrint;
    }
    public Tree(){}
    ...
    public void recPreOrder(Tree <T> node){
        if(node == null) return;
        listForPrint.add(node.val);
        recPreOrder(node.left);
        recPreOrder(node.right);
    }
    public void recInOrder(Tree<T> node){
        if(node == null) return;
        recInOrder(node.left);
        listForPrint.add(node.val);
        recInOrder(node.right);
    }
    public void recPostOrder(Tree <T> node){
        if(node == null) return;
    ...
    private void _printF(Tree<T> node){
        if(node == null) return;
        _print(node.left);
        listForPrint.add(node.val);
        //System.out.print(node + " ");
    ...
    public String printFrame(Tree<T> tree)
    {
        tree.printF();
        String str="";
        for(int i=0;i<tree.getListForPrint().size();i++) {
            str+=tree.getListForPrint().get(i).toString()+" ";
        }
        return str;
    }
